...
const WHISPER_OPTIONS = [
  {
    value: "FAST",
    label: "Cepat",
    sublabel: "Whisper Small",
    description: "Lebih cepat, akurasi standar. Cocok untuk kuliah ringan.",
  },
  {
    value: "BALANCED",
    label: "Seimbang",
    sublabel: "Whisper Medium",
    description: "Default. Keseimbangan terbaik antara kecepatan dan akurasi.",
  },
  {
    value: "ACCURATE",
    label: "Akurat",
    sublabel: "Whisper Large v3",
    description:
      "Paling akurat, sedikit lebih lambat. Ideal untuk presentasi padat.",
  },
];
...
export default function SettingsAIPage() {
  ...
  const selectClass =
    "bg-[var(--bg-elevated)] border border-[var(--border-default)] hover:border-[#A7D7C5] focus:border-[#74B49B] focus:ring-2 focus:ring-[rgba(167,215,197,0.25)] rounded-xl px-3 py-2 text-sm text-slate-800 dark:text-slate-100 focus:outline-none transition-all appearance-none cursor-pointer";
    ...
    <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
      <div className="mb-8">
        <h2
          ...
          style={{
            background: "linear-gradient(135deg,#74B49B,#A7D7C5)",
        ...
        <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">
          Customize how Fren-Edu records, transcribes, and summarizes your
          sessions.
        </p>
      </div>
      ...
      <SettingsGroup title="Transkripsi">
        ...
        <SettingsRow
          label="Kualitas Transkripsi"
          description="Pilih model Whisper yang sesuai dengan kebutuhanmu."
        >
      ...
      <SettingsGroup title="Rekaman">
        <SettingsRow
          ...
          description="Kualitas tinggi menghasilkan transkripsi lebih akurat."
          htmlFor="audio_quality"
        >
          <select
            id="audio_quality"
            value={audioQuality}
        ...
        <SettingsRow
          label="Interval Pemrosesan"
          description="Seberapa sering audio dikirim ke server untuk ditranskripsi."
          htmlFor="chunk"
        >
          <select
            id="chunk"
            value={chunkInterval}
            onChange={(e) => {
              setChunkInterval(Number(e.target.value));
              mark();
            }}
            className={selectClass}
          >
            {CHUNK_INTERVALS.map((o) => (
              <option key={o.value} value={o.value}>
                {o.label}
              </option>
            ))}
        ...
        <SettingsRow
          label="Peredam Kebisingan"
          description="Kurangi suara latar sebelum transkripsi."
          htmlFor="noise"
        >
          <Toggle
            id="noise"
      ...
      <SettingsGroup title="Ringkasan AI">
        <SettingsRow
          ...
          description="Buat ringkasan secara otomatis saat rekaman selesai."
          htmlFor="auto_sum"
        >
          <Toggle
            id="auto_sum"
        ...
        <SettingsRow
          ...
          description="Seberapa detail ringkasan yang dihasilkan."
          htmlFor="sum_len"
        >
          <select
            id="sum_len"
        ...
        <SettingsRow
          ...
          description="Pilih gaya penulisan ringkasan favoritmu."
          htmlFor="sum_fmt"
        >
          <select
            id="sum_fmt"
            value={summaryFormat}
            onChange={(e) => {
              setSummaryFormat(e.target.value);
              mark();
            }}
            className={selectClass}
            disabled={!autoSummarize}
          >
            {SUMMARY_FORMATS.map((o) => (
              <option key={o.value} value={o.value}>
                {o.label}
              </option>
            ))}
