...
function summarizeText(text, maxSentences = 3) {
  if (!text || text === "—") {
    return { summary: "Tidak ada teks untuk diringkas.", keyPoints: [] };
  }

  const sentences = text.match(/[^.!?]+[.!?]?/g) || [text];
  const sorted = sentences.slice().sort((a, b) => b.length - a.length);
  const summary = sorted
    .slice(0, Math.min(maxSentences, sorted.length))
    .join(" ");
...
ringkasBtn.onclick = () => {
  ...
  if (!text || text === "—") {
    alert("📝 Silakan buat rekaman dan tunggu transkrip terlebih dahulu.");
    return;
  }
...
function findBestAnswer(question, text) {
  if (!text || text === "—") return "Belum ada transkrip.";
  ...
  sentences.forEach((s) => {
    ...
    qWords.forEach((w) => {
      if (sw.includes(w)) score += 1;
    });
    if (score > best.score) best = { score, sent: s };
  });
  if (best.score === 0) {
    return "Maaf, jawaban tidak ditemukan dalam transkrip. Coba ringkas terlebih dahulu.";
  }
...
generateQaBtn.onclick = () => {
  const text = lastText || transcriptDiv.textContent;
  if (!text || text === "—") {
    alert("📝 Silakan buat rekaman terlebih dahulu.");
    return;
  }
  const { keyPoints } = summarizeText(text, 4);
  if (!keyPoints.length) {
    generatedQuestionsDiv.textContent =
      "Tidak ada poin untuk dijadikan pertanyaan.";
    return;
  }
  const qs = keyPoints
    .map((p, i) => {
      return `<div class="gen-q"><b>Q${i + 1}:</b> Jelaskan point ini: "${p}"</div>`;
    })
    .join("");
...
function populateVoices() {
  ...
  voiceSelect.innerHTML = voices
    .filter((v) => v.lang.includes("id") || v.lang.includes("en"))
...
speakSummaryBtn.onclick = () => {
  ...
  if (!text || text === "—") {
    alert("📝 Silakan ringkas transkrip terlebih dahulu.");
    return;
  }
...
console.log("🎓 Fren-Edu Prototype loaded!");
